# This file provides the runtime support for running a UNILA program
# Assumes the program has been parsed using UNILAparse.py
...
class UNILAInterpreter:
    ...
    def eval(self,expr):
        ...
        elif etype == 'STRING': return expr[1][1:len(expr[1])-1];
    ...
    def run(self):
        ...
        while 1:            
            ...
            # TODO: can be removed, we have no end operator
            ...
            # SHELL statement
                 ...
                 command = str(self.eval(instr[1]));
                 l = len(command);
                 if command[0] == '"' :
                     command = command[1:l-1];
                 os.system("%s" % command);
                 
            # CONNECT statement
            elif op == 'CONNECT':
                db_user = str(self.eval(instr[1]))
                db_host = str(self.eval(instr[2]))
                db_db = str(self.eval(instr[3]))
                db_passwd = str(self.eval(instr[4]))
            ...
            # QUERY statement
                ...
                db_query = str(self.eval(instr[1]))
                
                try:
                    self.curs.execute(db_query)
