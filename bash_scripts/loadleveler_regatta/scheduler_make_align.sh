...
MAKE=gmake
PARUS_HOME=${HOME}/parus-1.2.9_pre
...
then 
	...
	then
		...
		if ! $SUBMIT_COMMAND -np $num_procs -w 00:$minutes_to_work:00 ./prog > sbm.txt
		then
			echo "Submit to queue failed!"
			err
		fi
...
# regatta 
#
job_id=`grep "has been submitted" sbm.txt | cut -f 3  -d '.' | cut -f 1 -d '"' `
echo $job_id
